...
| **Agent group** | One of 4 parallel schema auditor partitions (Group A–D). |
| **Decisions ledger** | Persistent file at `.scratch/<auditor-name>/audit-decisions.md` recording approve/reject/skip decisions. |
...
7. **Deep audits are standalone** — Auditors run against the current state, not a diff.
