...
17. **No CHECK constraints on TEXT columns** — CHECK constraints on TEXT columns are explicitly prohibited. They cause significant LLM quality degradation (opaque, unrecoverable constraint errors) and migration friction (SQLite requires full table recreation to alter constraints). Validation of enum-like values belongs in the application layer (`write-tools.js` handlers) or is the agent's responsibility. The only permitted CHECK is `CHECK(id = 1)` on `project.id` (INTEGER PK singleton enforcement). Critics must not flag the absence of TEXT CHECK constraints as an issue or recommend adding/restoring them.
