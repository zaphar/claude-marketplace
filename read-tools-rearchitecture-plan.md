...
Remove the entire `attachRelated` function. Run `node --test test/` — all 97 tests must pass.

---

## Phase 3: Tests

### Step 3a: New test file `test/query-dispatch.test.js`
...
### Step 3b: Per-function filter tests (37 tests)

Each entity type gets at least one test that:
...
Example tests for representative types:

**persona (TEXT PK, simple filters):**
```js
it("queryPersona filters by name", () => {
  handleWriteTool("changelog_insert", {
    entity_type: "persona", iteration_id: seed.iteration_id,
    revision_id: seed.revision_id,
    data: { id: "P-1", name: "Alice", description: "Dev" }
  });
  handleWriteTool("changelog_insert", {
    entity_type: "persona", iteration_id: seed.iteration_id,
    revision_id: seed.revision_id,
    data: { id: "P-2", name: "Bob", description: "PM" }
  });
  const r = handleReadTool("changelog_query", {
    entity_type: "persona", filters: { name: "Alice" }
  });
...
**blocker (INTEGER PK, nullable filters):**
```js
it("queryBlocker filters by severity and null resolved_at", () => {
  handleWriteTool("changelog_insert", {
    entity_type: "blocker", iteration_id: seed.iteration_id,
    revision_id: seed.revision_id,
    data: { phase_name: "implementation", description: "Blocked", severity: "high", raised_by: "dev" }
  });
  const r = handleReadTool("changelog_query", {
    entity_type: "blocker", filters: { severity: "high", resolved_at: null }
  });
  assert.strictEqual(r.count, 1);
  assert.strictEqual(r.results[0].severity, "high");
  assert.strictEqual(r.results[0].resolved_at, null);
});
```

**adr (TEXT PK, nullable superseded_by — the field that triggered the original bug):**
```js
it("queryAdr filters by null superseded_by", () => {
  handleWriteTool("changelog_insert", {
    entity_type: "adr", iteration_id: seed.iteration_id,
    revision_id: seed.revision_id,
    data: { id: "ADR-1", title: "Use REST", status: "accepted", decision: "REST", rationale: "Simple" }
  });
  const r = handleReadTool("changelog_query", {
    entity_type: "adr", filters: { superseded_by: null }
  });
  assert.strictEqual(r.count, 1);
...
Remaining 34 entity types follow the same pattern with type-appropriate columns.

### Step 3c: Unknown filter key tests
...
### Step 3d: Null filter on non-nullable column test
...
### Step 3e: ids + filters combination test (covers audit Findings #1 and #9)
...
it("combines ids with null filters correctly", () => {
  ...
  // Query both ADRs but filter for superseded_by IS NULL
  const r = handleReadTool("changelog_query", {
    entity_type: "adr",
    ids: ["ADR-1", "ADR-2"],
...
### Step 3f: Run full test suite

`node --test test/` — all existing 97 tests + all new tests must pass.
