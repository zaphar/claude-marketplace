...
If `current_iteration` is null or the iteration's status is not `"active"`, stop with an error:
...
- The replan log at `<process_directory>/planning/iteration-<iteration_id>/replan-log.md` records what changed and why
