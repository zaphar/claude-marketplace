...
changelog_insert(entity_type: "blocker", iteration_id: <id>, data: {
  ...
  raised_by: "performance-audit-critic"  // required: agent name
})
```

**`changelog_update`** — only `status` is updatable; supported entity types:
```
changelog_update(entity_type: "performance_audit_finding", entity_id: <id>, updates: { status: "deferred" })
// performance_audit_finding statuses: "open" | "resolved" | "accepted" | "deferred"
